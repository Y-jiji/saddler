...
Both ship in this skill's `hooks/`. Copy them into `<project>/.claude/hooks/` and register in `<project>/.claude/settings.json`, so the rule is committed and shared with the team.
