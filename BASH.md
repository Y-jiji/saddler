...
`edit_guard.py` — PreToolUse on Bash. Bash can write only existing gitignored paths in the project repo, `/tmp` and `~/.cache`; everything else, including tracked files, untracked files, `.git` and the home directory, is read-only. An ignored path that does not exist yet cannot be created unless its parent is writable. A plain `git ...` command can also write the whole repo, so checkout and stash pop still work. Outside a git repo only `/tmp` and `~/.cache` are writable. Refuses the command above 2900 binds.
