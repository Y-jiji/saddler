...
Place both in `<project>/hooks/`, register in `<project>/.claude/settings.json`. Use `~/.claude/` for both instead to cover every project, with the paths spelled out.

Hook commands, matcher `Bash`:
- PreToolUse — `uv run --quiet "$CLAUDE_PROJECT_DIR/hooks/edit_guard.py" || exit 2`
- PostToolUse and PostToolUseFailure — `python3 "$CLAUDE_PROJECT_DIR/hooks/edit_hint.py"`
